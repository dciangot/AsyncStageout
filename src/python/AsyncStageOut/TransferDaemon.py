...
class TransferDaemon(BaseDaemon):
    ...
    def algorithm(self, parameters = None):
        ...
        for u in users:
	     if 'jmsilva' not in u: #any(u[0].find(c) for c in ("betty", "wilma", "barney", "fred")):	
    ...
    def active_users(self, db):
        ...
        if len(users['rows']) <= self.config.pool_size:
            ...
            sorted_users = self.factory.loadObject(self.config.algoName, args = [self.config, self.logger,
                                                                                 users['rows'], self.config.pool_size],
